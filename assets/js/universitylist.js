...
function filterUniversities() {
    ...
    if (searchFiltered.length === 0) {
        ...
        document.querySelector(".pagination").innerHTML = ""; // kosongkan pagination juga
    } else {
...
document.getElementById('menuBtn').addEventListener('click', function () {
    this.classList.toggle('active'); // Toggle kelas aktif pada tombol
  });
...
// Deteksi klik di luar untuk menutup menu
// Deteksi klik di luar untuk menutup menu dan filter
document.addEventListener('click', function (e) {
    const menuButton = document.getElementById("menuBtn");
    const menuDropdown = document.getElementById("menuDropdown");
    const filterButton = document.getElementById("filterBtn"); // <-- Tambahan
    const filterPanel = document.getElementById("filterPanel");
    const overlay = document.querySelector(".custom-overlay");

    const isClickInsideMenu = menuButton.contains(e.target) || menuDropdown.contains(e.target);
    const isClickInsideFilter = filterButton.contains(e.target) || filterPanel.contains(e.target); // <-- Tambahan

    if (!isClickInsideMenu && !isClickInsideFilter) {
